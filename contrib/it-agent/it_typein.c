...
static int64_t call(unsigned op, void *buffer, unsigned offset, unsigned length)
{
    ...
    memset(&q,0,sizeof(q));
    q.op=op;q.address=(uint32_t)buffer;q.offset=offset;q.length=length;q.token=cookie;
...
static void tick(ID timer,void *unused)
{
    ...
    if(!app)goto out;
    int64_t pending=call(0x166,0,pid,0);
    if(pending>0) {
        cookie=pending;
        ...
        if(!op)status=-EINVAL;
        if(!status && !is_foreground())status=-EAGAIN;
        if(!status) {
            *nl=0;op++;
    ...
    /* Only the foreground app with a keyboard may consume host key events.
    ...
    if(!(unsigned)msg(app,sel("isSuspended")) && call(0x131,0,0,0)>0) {
        ID keyboard=msg(getClass("UIKeyboardImpl"),sel("activeInstance"));
        if(keyboard && msg(keyboard,sel("delegate")) && is_foreground()) {
...
static void *setup(void *unused)
{
    ...
    if(!releaseCF||!createBytes||!timerCreate||!addTimer||!mainLoop||!currentTime||!frontmost||!processID||!modes)return 0;
    pid=getpid();
    ...
    if(bundle && identifier && getString) {
        ...
        if(value && getString(value,name,sizeof(name),0x08000100) && strcmp(name,"com.apple.springboard"))delay=0.25;
    }
